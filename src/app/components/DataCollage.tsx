  ...
  return (
    <div className="relative w-[129px] flex flex-col items-center gap-[6px] -translate-x-1/2 -translate-y-1/2 -mt-[20px]">
      ...
      <div className="bg-white px-2 py-1 rounded-[8px] shadow-sm flex items-center justify-center min-w-[120px] -translate-y-[5px]">
        <span className="font-['Figtree'] font-bold text-[16px] text-black">
          Lumière Bistro
        </span>
      </div>
      ...
      >
        {/* Union Shape */}
        <svg
          className="absolute top-0 left-0 w-[79px] h-[86px]"
          viewBox="0 0 79 86"
          fill="none"
        >
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 text-slate-200">
                  <RotateCw
                    className={`w-5 h-5 ${isRotating ? "text-blue-400" : "text-slate-500"}`}
                  />
                ...
                >
                  <div
                    className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow-sm transition-transform ${isRotating ? "left-6" : "left-1"}`}
                  />
                </button>
              </div>
            </motion.div>
      ...
      <div className="bg-slate-900/60 backdrop-blur-md rounded-3xl overflow-hidden border border-white/10 shadow-sm">
        <div className="h-64 relative">
          ...
          <div className="absolute bottom-8 left-8 text-white">
            ...
            <p className="opacity-90 flex items-center gap-2 text-lg font-light">
              <MapPin className="w-5 h-5" /> 123 Culinary Ave,
              Food District, NY
            </p>
          </div>
        </div>
        <div className="p-10 flex items-start justify-between">
          <div className="flex gap-12">
            <div>
              <span className="text-xs text-slate-400 uppercase tracking-widest font-bold mb-2 block">
                Google Rating
              </span>
  ...
  const reviews = [
    {
      ...
      time: "2 months ago",
      rating: 5,
      text: "First time I came here was in the 2000's. My cousin works nearby and told me about it. It just to be a shared spot. They only operate in evening, duri...",
      avatar: avatars[0],
    },
    {
      name: "Chanil Jung",
      time: "3 months ago",
      rating: 5,
      text: "I tried The Halal Guys from their street food cart in NYC, and honestly, it's unbeatable for the price. Huge portions, great flavor, and super satisfy...",
      avatar: avatars[1],
    },
    {
      name: "G 3956",
      time: "4 months ago",
      rating: 5,
      text: "I've heard about Halal Guys for many years, and food blogs always said it was good, but I'm usually not the type of person to go for this kind of food...",
      avatar: avatars[2],
    },
    {
      name: "Nabiha K.",
      time: "4 weeks ago",
      rating: 5,
      text: "Can you believe 1 hr and 40 minutes drive just for this spot??? Yup! Been doing it for 20 yrs and will keep going as long as they stay open! Only thin...",
      avatar: avatars[3],
    },
    {
      name: "Richard Yu",
      time: "4 months ago",
      rating: 4,
      text: "The Halal Guys are a true NYC staple — iconic, really. It's still one of the best food buys out there, though the quality has been somewhat inconsiste...",
      avatar: avatars[4],
    },
  ];
        ...
        {reviews.map((review, i) => (
          ...
          >
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="flex text-amber-400 gap-0.5">
                {Array.from({ length: 5 }).map((_, idx) => (
                  <Star
                    key={idx}
                    className={`w-4 h-4 ${idx < review.rating ? "fill-current" : "text-slate-800 fill-slate-800"}`}
                  />
                ))}
              </div>
            </div>
            <p className="text-slate-300 text-sm leading-relaxed">
              {review.text}{" "}
              <span className="text-slate-500 font-medium cursor-pointer hover:text-blue-400">
                More
              </span>
            </p>
          </motion.div>
  ...
  return (
    ...
    >
      <div className="relative mb-8">
        ...
        <div className="absolute inset-0 flex items-center justify-center font-bold text-3xl text-blue-500">
          85%
        </div>
      </div>
...
function AiAssistantOverlay({ stepId }: { stepId: string }) {
  const [displayText, setDisplayText] = useState("");

  useEffect(() => {
    if (stepId === 'location') {
        setDisplayText("Finding nearby competitors...");
        
        // Match the delays in LocationView: 0, 3, 6, 9, 12, 15, 18, 21 seconds
        // Plus the initial 1s delay before they start showing
        ...
        delays.forEach((d, index) => {
             const t = setTimeout(() => {
                 ...
                 setDisplayText(`Found ${newCount} competitor${newCount !== 1 ? 's' : ''} nearby`);
             }, startDelay + (d * 1000));
             timeouts.push(t);
        });
        ...
        switch(stepId) {
            case 'profile':
                setDisplayText("Analyzing business profile...");
                break;
            case 'sentiment':
                setDisplayText("Scanning customer reviews...");
                break;
            case 'photos':
                setDisplayText("Auditing visual assets...");
                break;
            case 'report':
                setDisplayText("Compiling final report...");
                break;
            default:
                setDisplayText("AI Assistant Active");
        }
    }
  }, [stepId]);

  return (
     <div className="absolute bottom-12 left-1/2 -translate-x-1/2 bg-slate-900/60 backdrop-blur-xl px-6 py-3 rounded-full shadow-2xl border border-white/10 flex items-center gap-4 z-50 transition-all duration-300 hover:bg-slate-900/70">
         <SiriWaveIcon className="w-8 h-8" />
         <motion.span 
            key={displayText}
            initial={{ opacity: 0, y: 5 }}
            animate={{ opacity: 1, y: 0 }}
            className="font-medium text-slate-200 text-sm tracking-wide whitespace-nowrap"
         >
            {displayText}
         </motion.span>
     </div>
...
export function DataCollage({ stepId }: AuditContentProps) {
  return (
    <div className="w-full h-full flex items-center justify-center relative">
      ...
      <AiAssistantOverlay stepId={stepId} />
    </div>
  );
}